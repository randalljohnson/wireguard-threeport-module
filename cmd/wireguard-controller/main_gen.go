...
import (
	...
	wireguard "github.com/randalljohnson/wireguard-threeport-module/internal/wireguard"
...
func main() {
	// flags
	var wireguardDefinitionConcurrentReconciles = flag.Int(
		"wireguard-definition-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for wireguard definitions",
	)
	var wireguardInstanceConcurrentReconciles = flag.Int(
		"wireguard-instance-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for wireguard instances",
	)
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      wireguard.LockBucketName,
		Description: wireguard.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", wireguard.LockBucketName)
		os.Exit(1)
	}

	// check to ensure wireguard stream has been created by API
	wireguardStreamNameFound := false
	for stream := range js.StreamNames() {
		if stream == notif.WireguardStreamName {
			wireguardStreamNameFound = true
		}
	}
	if !wireguardStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with wireguard stream name", "wireguardStreamName", notif.WireguardStreamName)
		os.Exit(1)
	}
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *wireguardDefinitionConcurrentReconciles,
		Name:                 "WireguardDefinitionReconciler",
		NotifSubject:         notif.WireguardDefinitionSubject,
		ReconcileFunc:        wireguard.WireguardDefinitionReconciler,
	})
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *wireguardInstanceConcurrentReconciles,
		Name:                 "WireguardInstanceReconciler",
		NotifSubject:         notif.WireguardInstanceSubject,
		ReconcileFunc:        wireguard.WireguardInstanceReconciler,
	})
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(notif.WireguardStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.WireguardStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "WireguardController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info(
		...
		"lockBucketName", wireguard.LockBucketName,
	)
	...
	log.Info("wireguard controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-wireguard-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
